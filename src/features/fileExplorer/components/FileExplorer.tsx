...
const FileExplorer: React.FC<FileExplorerProps> = ({
  ...
  const loadTopLevelFolders = useCallback(async () => {
    ...
    try {
      ...
      toast("Error loading folders", {
        description:
          "There was an error loading the top-level folders. Please try again.",
      });
  ...
  const handleAddTopLevelFolder = useCallback(async () => {
    try {
      ...
      if (selectedFolder) {
        const updatedFolders = await window.electron.addTopLevelFolder(
          selectedFolder
        );
        setTopLevelFolders(updatedFolders);
        toast("Folder added", {
          description: `The folder "${getFolderName(
            selectedFolder
          )}" has been added as a top-level folder.`,
        });
      }
    } catch (error) {
      ...
      toast("Error adding folder", {
        description:
          "There was an error adding the top-level folder. Please try again.",
      });
    }
  }, []);
  ...
  const renderDirectoryStructure = useCallback(
    (structure: DirectoryStructure, currentPath = "") => {
    ...
    >
      ...
      {isAddingTopLevelFolder && (
        <div className="flex items-center mt-2 p-2">
          ...
          <Button
            variant="ghost"
            onClick={() => setIsAddingTopLevelFolder(false)}
          >
            Cancel
          </Button>
        </div>
      ...
      {folderCreationError && (
        <div className="text-red-500 text-sm p-2">{folderCreationError}</div>
      )}
