...
const NOTES_DIR = path.join(process.env.HOME || process.env.USERPROFILE || '', '.my-notes-app');
...
export const setupFileSystem = () => {
  ...
  ipcMain.handle('save-note', async (_, note) => {
    try {
      await fs.writeFile(path.join(NOTES_DIR, `${note.id}.json`), JSON.stringify(note));
    } catch (error) {
